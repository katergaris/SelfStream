...
async function getMovieTitleFromTmdb(tmdbId: string): Promise<{ title: string; year?: number } | null> {
...
function pickBestPostLink(links: string[], title: string): string | null {
...
function extractEmbedAjax(jsonText: string): string | null {
    const m = jsonText.match(/"embed_url"\s*:\s*"([^"]+)"/i)?.[1];
    if (!m) return null;
    return m.replace(/\\\//g, '/').replace('/wws.', '/v2.');
}

async function resolveFinalUrl(startUrl: string): Promise<string | null> {
    try {
        ...
        if (/\.m3u8(\?|$)/i.test(finalUrl) || /\.mp4(\?|$)/i.test(finalUrl)) return finalUrl;
...
export async function getCb01Streams(tmdbId: string): Promise<Array<{ name: string; title: string; url: string }>> {
    try {
        ...
        if (/\.m3u8(\?|$)/i.test(finalUrl)) {
            const token = makeProxyToken(finalUrl, {
                'User-Agent': UA,
                'Referer': `${CB01_URL}/`,
                'Origin': CB01_URL
            });
