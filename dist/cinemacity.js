...
exports.CINEMACITY_HEADERS = void 0;
exports.getCinemaCityStreams = getCinemaCityStreams;
exports.extractFreshStreamUrl = extractFreshStreamUrl;
/**
...
exports.CINEMACITY_HEADERS = HEADERS;
 ...
 */
function matchJsonArray(text, key) {
    ...
    const result = [arrayStr, arrayStr];
...
function extractFileData(html) {
    ...
    $('script').each((_i, el) => {
        ...
        if (!scriptHtml || !scriptHtml.includes('atob'))
            return;
        ...
        while ((match = regex.exec(scriptHtml)) !== null) {
            ...
            if (fileMatch) {
                ...
                console.log('[CinemaCity] File data extracted, type:', Array.isArray(fileData) ? `array[${fileData.length}]` : typeof fileData, Array.isArray(fileData) && fileData[0]?.folder ? '(has folders)' : '');
            }
        }
    });
    return fileData;
...
function parseSubtitles(subtitleStr) {
    if (!subtitleStr || typeof subtitleStr !== 'string')
        return [];
    const tracks = [];
 ...
 */
function pickSubtitleStr(fileData, type, season = 1, episode = 1) {
    if (!Array.isArray(fileData))
        return '';
    // Flat array (movie)
    if (type === 'movie' || fileData.every((x) => x && typeof x === 'object' && 'file' in x && !('folder' in x))) {
 ...
 */
function resolveAbsUrl(base, rel) {
 ...
 */
function buildStreamHeaders(playerReferer, pageUrl) {
    let origin;
    try {
 ...
 */
async function searchCinemaCity(query, exactMatch = true) {
    ...
    items.each((_i, el) => {
        ...
        if (foundTitle.toLowerCase().includes(query.toLowerCase()) ||
 ...
 */
async function getCinemaCityStreams(tmdbId, mediaType, season, episode, preferredLang) {
    try {
        ...
        if (tmdbId.startsWith('tt')) {
            ...
            try {
                const findData = await fetchJson(`https://api.themoviedb.org/3/find/${tmdbId}?api_key=${TMDB_API_KEY}&external_source=imdb_id`);
            ...
            if (lang !== 'en') {
                try {
                    const findData = await fetchJson(`https://api.themoviedb.org/3/find/${tmdbId}?api_key=${TMDB_API_KEY}&external_source=imdb_id&language=${lang}`);
                    const results = findData?.movie_results?.[0] || findData?.tv_results?.[0];
            ...
            const tmdbData = await fetchJson(`https://api.themoviedb.org/3/${tmdbType}/${tmdbId}?api_key=${TMDB_API_KEY}&append_to_response=external_ids`);
            imdbId = tmdbData?.imdb_id || tmdbData?.external_ids?.imdb_id || null;
            titleEn = tmdbData?.title || tmdbData?.name || null;
            if (lang !== 'en') {
                try {
                    const langData = await fetchJson(`https://api.themoviedb.org/3/${tmdbType}/${tmdbId}?api_key=${TMDB_API_KEY}&language=${lang}`);
        ...
        const tokenData = { page: mediaUrl };
        if (season)
            tokenData.s = parseInt(season, 10);
        if (episode)
            tokenData.e = parseInt(episode, 10);
        if (lang)
            tokenData.lang = lang;
 ...
 */
async function extractFreshStreamUrl(pageUrl, season, episode) {
    try {
...
//# sourceMappingURL=cinemacity.js.map