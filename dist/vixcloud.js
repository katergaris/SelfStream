...
exports.getVixCloudStreams = getVixCloudStreams;
...
async function resolveFromMapping(kitsuId, episodeNum) {
    try {
        const ep = parseInt(episodeNum) || 1;
        const url = `${ANIMEMAPPING_BASE}/kitsu/${kitsuId}?ep=${ep}`;
        console.log(`[VixCloud] Fetching mapping: ${url}`);
        const { body, statusCode } = await (0, undici_1.request)(url, { headers: { 'Accept': 'application/json' } });
        if (statusCode !== 200) {
...
async function resolveEpisodeFromMapping(kitsuId, episodeNum) {
    try {
        const ep = parseInt(episodeNum) || 1;
        const url = `${ANIMEMAPPING_BASE}/kitsu/${kitsuId}?ep=${ep}`;
        const { body, statusCode } = await (0, undici_1.request)(url, { headers: { 'Accept': 'application/json' } });
        ...
        if (fromKitsu && typeof fromKitsu === 'number' && fromKitsu > 0)
            return fromKitsu;
        const fromRequested = data?.requested?.episode;
        if (fromRequested && typeof fromRequested === 'number' && fromRequested > 0)
...
async function getKitsuTitle(kitsuId) {
    try {
        const { body, statusCode } = await (0, undici_1.request)(`https://kitsu.io/api/edge/anime/${kitsuId}`);
        if (statusCode !== 200)
            return null;
        const data = await body.json();
...
async function getAnimeUnitySession() {
    const { body, headers } = await (0, undici_1.request)(AU_BASE, {
...
async function searchAnimeUnity(title, session) {
    const { body } = await (0, undici_1.request)(`${AU_BASE}/livesearch`, {
        method: 'POST',
        headers: {
            'User-Agent': AU_UA,
...
async function getEmbedUrl(animePath, episodeNum) {
    ...
    const targetEp = parsedEpisodes.find((e) => {
    ...
    const { body: epBody } = await (0, undici_1.request)(epPageUrl, {
        headers: { 'User-Agent': AU_UA }
    });
...
async function extractVixCloudManifest(embedUrl) {
    ...
    const { body, statusCode } = await (0, undici_1.request)(embedUrl, {
        headers: proxy_1.VIXCLOUD_HEADERS
    });
    let html = "";
    if (statusCode === 200) {
    ...
    if (masterPlaylistMatch?.groups) {
        ...
        if (urlMatch)
            playlistUrl = urlMatch[1].replace(/\\/g, '');
        if (tMatch && !token)
            token = tMatch[1];
        if (eMatch && !expires)
            expires = eMatch[1];
...
// ── Main entry point ──
async function getVixCloudStreams(kitsuId, episodeNumber = "1") {
    try {
        ...
        const proxyToken = (0, proxy_1.makeProxyToken)(manifestUrl, proxy_1.VIXCLOUD_HEADERS);
        return [{
                name: "AU 🤌",
                title: "VIX 1080 🤌",
...
//# sourceMappingURL=vixcloud.js.map